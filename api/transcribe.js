...
const handler = async (req, res) => {
    ...
    if (!url) return res.status(400).json({ creator: 'Doo', status: false, message: 'Parameter url atau id diperlukan' });
    ...
    try {
        ...
        return res.status(500).json({ creator: 'Doo', status: false, message: err.message });
    }
};
