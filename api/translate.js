...
const handler = async (req, res) => {
    ...
    try {
        if (lines && Array.isArray(lines)) {
            ...
            return res.status(200).json({ status: true, result: { lines: translatedLines, creator: 'Doo' } });
        } else if (text) {
            const translatedText = await translateText(text, targetLang);
            return res.status(200).json({ status: true, result: { original: text, translated: translatedText, targetLang, creator: 'Doo' } });
        }
    } catch(e) {
