...
app.use((req, res) => {
    ...
    if (req.path.startsWith('/play/')) {
        ...
        if (videoId) {
            ...
            const playTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist}` : qTitle) : `Dengarkan Musik - DooMusify`;
            const playDesc = `Dengarkan ${qTitle || 'lagu favoritmu'} di DooMusify Web Music Player`;
    ...
    if (req.path.startsWith('/artist/')) {
        ...
        if (artistId) {
            ...
            const pageTitle = qName ? `${qName} (Artist) - DooMusify` : `Artist - DooMusify`;
            const pageDesc = qName ? `Dengarkan lagu & album terbaik dari ${qName} di DooMusify` : `Dengarkan lagu & album dari artist favoritmu di DooMusify`;
    ...
    if (req.path.startsWith('/album/')) {
        ...
        if (albumId) {
            ...
            const pageTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist} (Album) - DooMusify` : `${qTitle} (Album) - DooMusify`) : `Album - DooMusify`;
            const pageDesc = qTitle ? `Dengarkan album ${qTitle} di DooMusify` : `Dengarkan album favoritmu di DooMusify`;
            const coverUrl = qCover || `https://www.gobox.my.id/file/RztXot0DT0Q1.png`;
